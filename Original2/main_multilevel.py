"""
・MOSEのmultilevelを参考に実装
・損失は教師あり対照損失と交差エントロピー損失の２種類で蒸留損失は不使用
"""
...
from networks.resnet_multilevel_cifar import resnet18_sd
from losses.loss_multilevel import SupConLoss
from optimizer.optimizer_multilevel import set_optimizer
from utils.util_multilevel import TwoCropTransform
from datasets.dataset_multilevel import TinyImagenet
from utils.util_multilevel import adjust_learning_rate
from utils.util_multilevel import warmup_learning_rate
from train.train_multilevel import train
from utils.util_multilevel import save_model
...
def parse_option():
    ...
    # modelのパラメータや記録の保存作
    opt.model_path = f'./exp_logs/checkpoints_multilevel/{opt.original_name}/model_param/'
    opt.log_path   = f'./exp_logs/checkpoints_multilevel/{opt.original_name}/buffer_log/'
    opt.learning_path = f'./exp_logs/checkpoints_multilevel/{opt.original_name}/learning_log/'
